...
class PauliString:
	...
	def from_zx_bits(cls, zx_bits: NDArray[np.bool_]) -> 'PauliString':
		...
		middle = len(zx_bits)//2
		z_bits = zx_bits[:middle]
		x_bits = zx_bits[middle:]
		################################################################################################################
	...
	def from_str(cls, pauli_str: str) -> 'PauliString':
		...
		################################################################################################################
		z_bits = np.array([1 if (letter == 'Z' or letter == 'Y') else 0 for letter in pauli_str], dtype=bool)[::-1]
		x_bits = np.array([1 if (letter == 'X' or letter == 'Y') else 0 for letter in pauli_str], dtype=bool)[::-1]
	...
	def mul_pauli_string(self, other: 'PauliString') -> 'PauliString':
		...
		w = 2*np.sum(other.z_bits * self.x_bits) + np.sum(self.z_bits*self.x_bits) + np.sum(other.z_bits*other.x_bits) - np.sum(new_z_bits*new_x_bits)
		phase = (-1j)**w
		################################################################################################################
	...
	def ids(self) -> NDArray[np.bool_]:
		...
		# YOUR CODE HERE
		# TO COMPLETE (after lecture on mapping)
		ids = [letter == 'I' for letter in str(self)]
		ids.reverse()
		################################################################################################################
...
class LinearCombinaisonPauliString:
	...
	def mul_linear_combinaison_pauli_string(self, other: 'LinearCombinaisonPauliString') -> 'LinearCombinaisonPauliString':
		...
		# TO COMPLETE (after lecture on mapping)
		k = 0
		for i, (coef1, pauli_str1) in enumerate(zip(self.coefs, self.pauli_strings)):
			for j, (coef2, pauli_str2) in enumerate(zip(other.coefs, other.pauli_strings)):
       
				new_pauli_strings[k], phase = pauli_str1*pauli_str2
				new_coefs[k] = phase*coef1*coef2
				k += 1
	...
	def to_zx_bits(self) -> NDArray[np.bool_]:
		...
		for i, pauli_str in enumerate(self.pauli_strings):
			zx_bits[i, :] = pauli_str.to_zx_bits()
		################################################################################################################
	...
	def to_xz_bits(self) -> NDArray[np.bool_]:
		...
		for i, pauli_str in enumerate(self.pauli_strings):
			xz_bits[i, :] = pauli_str.to_xz_bits()
		################################################################################################################
	...
	def combine(self) -> 'LinearCombinaisonPauliString':
		...
		new_coefs = new_pauli_strings = list()
	...
	def apply_threshold(self, threshold: float = 1e-6) -> 'LinearCombinaisonPauliString':
		...
		above_threshold = np.where(np.abs(self.coefs) > threshold)

		new_coefs = self.coefs[above_threshold]
		new_pauli_strings = self.pauli_strings[above_threshold]
	...
	def sort(self) -> 'LinearCombinaisonPauliString':
		...
		# YOUR CODE HERE
		# TO COMPLETE (after lecture on mapping)
		order = np.argsort(self.to_zx_bits()[:, 0], axis=0)
		################################################################################################################
