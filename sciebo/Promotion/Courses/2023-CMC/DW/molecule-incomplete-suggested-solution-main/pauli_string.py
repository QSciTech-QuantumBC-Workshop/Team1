...
class PauliString:
    ...
    def from_zx_bits(cls, zx_bits: NDArray[np.bool_]) -> 'PauliString':
        ...
        #print(zx_bits[2])
        zx_len = len(zx_bits)
        zx_len2 = int(zx_len/2)
        z_bits = np.zeros(zx_len2, dtype = np.bool_)
        x_bits = np.zeros(zx_len2, dtype = np.bool_)
        for j in range(0, zx_len2):
            #count += 1
            if (zx_bits[j] == True) and (zx_bits[zx_len2+j] == False):
               z_bits[j] = True
               x_bits[j] = False
            if zx_bits[j] == False and zx_bits[zx_len2+j] == True:
               z_bits[j] = False
               x_bits[j] = True
            if zx_bits[j] == True and zx_bits[zx_len2+j] == True:
               z_bits[j] = True
               x_bits[j] = True
            if zx_bits[j] == False and zx_bits[zx_len2+j] == False:
               z_bits[j] = False
               x_bits[j] = False
        #sys.exit()
        ################################################################################################################
    ...
    def from_str(cls, pauli_str: str) -> 'PauliString':
        ...
        lst = []
        count = 0
        for letter in pauli_str:
            count += 1
            lst.append(letter)
        length = count
        #print(lst)
        z_bits = np.zeros(length, dtype = np.bool_)
        x_bits = np.zeros(length, dtype = np.bool_)
        count = -1
        for letter in pauli_str:
            count += 1
            if letter =='Z':
               z_bits[length-count-1] = True
               x_bits[length-count-1] = False
            if letter =='X':
               z_bits[length-count-1] = False
               x_bits[length-count-1] = True
            if letter =='Y':
               z_bits[length-count-1] = True
               x_bits[length-count-1] = True
            if letter =='I':
...
################################################################################################################
    ...
    def to_zx_bits(self) -> NDArray[np.bool_]:
        ...
        length = count
        #print(lst)
        zx_bits = np.zeros(2*length, dtype = np.bool_)
        count = -1
        for letter in pstring:
            count += 1
            if letter =='Z':
               zx_bits[length-count-1] = True
               zx_bits[2*length-count-1] = False
            if letter =='X':
               zx_bits[length-count-1] = False
               zx_bits[2*length-count-1] = True
            if letter =='Y':
               zx_bits[length-count-1] = True
               zx_bits[2*length-count-1] = True
            if letter =='I':
               zx_bits[length-count-1] = False
               zx_bits[2*length-count-1] = False
        #xx, zz = self.from_str(x)
        ##print(self.from_str(x))
        ##sys.exit()
       ################################################################################################################
    ...
    def to_xz_bits(self) -> NDArray[np.bool_]:
        ...
        length = count
        #print(lst)
        xz_bits = np.zeros(2*length, dtype = np.bool_)
        count = -1
        for letter in pstring:
            count += 1
            if letter =='Z':
               xz_bits[2*length-count-1] = True
               xz_bits[length-count-1] = False
            if letter =='X':
               xz_bits[2*length-count-1] = False
               xz_bits[length-count-1] = True
            if letter =='Y':
               xz_bits[2*length-count-1] = True
               xz_bits[length-count-1] = True
            if letter =='I':
    ...
    def ids(self) -> NDArray[np.bool_]:
        ...
        length = count
        #print(lst)
        ids = np.zeros(length, dtype = np.bool_)
        count = -1
        for letter in pstring:
            count += 1
            if letter =='I':
               ids[length-count-1] = True
        ################################################################################################################
