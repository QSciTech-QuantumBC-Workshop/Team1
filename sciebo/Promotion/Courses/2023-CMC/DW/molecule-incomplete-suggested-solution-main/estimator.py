...
class Estimator:
    ...
    def diagonal_pauli_string_eigenvalue(diagonal_pauli_string: PauliString, state: str) -> float:
        ...
        # YOUR CODE HERE
        # TO COMPLETE (after lecture on VQE)
        dps = str(diagonal_pauli_string)
        eigenvalue = 1
        count = -1
        for letter in dps:
            count += 1
            if letter == 'Z' and state[count] == '1':
                eigenvalue *= -1
        #print(state2)
        #sys.exit()
            ################################################################################################################
    ...
    def estimate_diagonal_pauli_string_expectation_value(diagonal_pauli_string: PauliString, counts: dict) -> float:
        ...
        for key, value in counts.items():
            Ntot += value
            expectation_value+=value *Estimator.diagonal_pauli_string_eigenvalue(diagonal_pauli_string, key)
        expectation_value = expectation_value/Ntot    
    ################################################################################################################
    ...
    def diagonalizing_pauli_string_circuit(pauli_string: PauliString) -> tuple[QuantumCircuit, PauliString]:
        ...
        #w, V = np.linalg.eig(X_MAT)
        #D = np.matmul(V,np.matmul(X_MAT,np.linalg.inv(V)))
        #H = Operator(V)
        #print(V)
        #sys.exit()
        p_str = str(pauli_string)
        pstr_len = len(p_str)
        #diagonal_pauli_string = np.zeros(pstr_len, dtype = np.str)
        ...
        for letter in p_str:
            ...
            if letter != 'I':
                if letter == 'X':
                   diagonalizing_circuit.h(count)
                if letter == 'Y':
                   diagonalizing_circuit.sdg(count)
                #diagonal_pauli_string[count] = 'Z'
                diagonal_pauli_string += 'Z' 
            else:
                diagonal_pauli_string += 'I'               ################################################################################################################
