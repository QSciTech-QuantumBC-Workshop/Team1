...
class Estimator:
    ...
    def diagonalizing_pauli_string_circuit(pauli_string: PauliString) -> tuple[QuantumCircuit, PauliString]:
        ...
        # YOUR CODE HERE
        # TO COMPLETE (after lecture on VQE)
        #PauliString_copy = PauliString
        
        diagonal_pauli_string_string_rep = str(pauli_string).replace('X','Z').replace('Y', 'Z')
        diagonal_pauli_string = PauliString.from_str(diagonal_pauli_string_string_rep)
        
        for i, letter in enumerate(str(pauli_string)[::-1]):
            if (letter == 'Y'):
